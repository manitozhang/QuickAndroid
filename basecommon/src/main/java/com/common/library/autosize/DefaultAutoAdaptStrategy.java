 ...
 */
package com.common.library.autosize;

import android.app.Activity;
import android.app.Application;
import android.util.Log;

import com.common.library.autosize.external.ExternalAdaptInfo;
import com.common.library.autosize.internal.CancelAdapt;
import com.common.library.autosize.internal.CustomAdapt;
...
public class DefaultAutoAdaptStrategy implements AutoAdaptStrategy {
    ...
    private static final String TAG = DefaultAutoAdaptStrategy.class.getName();
    ...
    public void applyAdapt(Object target, Activity activity) {
        ...
        if (AutoSizeConfig.getInstance().getExternalAdaptManager().isRun()) {
            if (AutoSizeConfig.getInstance().getExternalAdaptManager().isCancelAdapt(target.getClass())) {
                Log.w(TAG, String.format(Locale.ENGLISH, "%s canceled the adaptation!", target.getClass().getName()));
                ...
                if (info != null) {
                    Log.d(TAG, String.format(Locale.ENGLISH, "%s used %s for adaptation!", target.getClass().getName(), ExternalAdaptInfo.class.getName()));
        ...
        if (target instanceof CustomAdapt) {
            Log.d(TAG, String.format(Locale.ENGLISH, "%s implemented by %s!", target.getClass().getName(), CustomAdapt.class.getName()));
            AutoSize.autoConvertDensityOfCustomAdapt(activity, (CustomAdapt) target);
        } else {
            Log.d(TAG, String.format(Locale.ENGLISH, "%s used the global configuration.", target.getClass().getName()));

...
import com.common.library.bravh_rvadapter.animation.AlphaInAnimation;
import com.common.library.bravh_rvadapter.animation.BaseAnimation;
import com.common.library.bravh_rvadapter.animation.ScaleInAnimation;
import com.common.library.bravh_rvadapter.animation.SlideInBottomAnimation;
import com.common.library.bravh_rvadapter.animation.SlideInLeftAnimation;
import com.common.library.bravh_rvadapter.animation.SlideInRightAnimation;
import com.common.library.bravh_rvadapter.entity.IExpandable;
import com.common.library.bravh_rvadapter.loadmore.LoadMoreView;
import com.common.library.bravh_rvadapter.loadmore.SimpleLoadMoreView;
import com.common.library.bravh_rvadapter.util.MultiTypeDelegate;
 ...
 */
public abstract class BaseRecyclerAdapter<T, K extends RecyclerViewHolder> extends RecyclerView.Adapter<K> {
    ...
    private boolean emptyInsert = false;
     ...
     * @deprecated This method is because it can lead to crash: always call this method while RecyclerView is computing a layout_game_detail_outs_more_image_progress_bar or scrolling.
     ...
     * simple to solve item will layout_game_detail_outs_more_image_progress_bar using all
    ...
    /**
     * When set to true, the item will layout_game_detail_outs_more_image_progress_bar using all span area. That means, if orientation
    ...
    protected K createBaseViewHolder(View view) {
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            k = (K) new RecyclerViewHolder(view);
        } else {
            k = createGenericKInstance(z, view);
        }
        return k != null ? k : (K) new RecyclerViewHolder(view);
    }
        ...
        if (type instanceof ParameterizedType) {
            ...
            for (Type temp : types) {
                if (temp instanceof Class) {
                    ...
                    if (rawType instanceof Class && RecyclerViewHolder.class.isAssignableFrom((Class<?>) rawType)) {
    ...
    /**
     * Return root layout_game_detail_outs_more_image_progress_bar of header
     */
    ...
    /**
     * Return root layout_game_detail_outs_more_image_progress_bar of footer
     */
    ...
    /**
     * 添加头布局
     * 重新封装,为了防止一个页面多个布局重复代码逻辑,这样每次进来重新实例化
     *
     * @param context;
     * @param mLayoutResId: 布局
     */
    public View setDefaultMoreHeader(Context context, int mLayoutResId) {
        View inflate = LayoutInflater.from(context).inflate(mLayoutResId, null);
     ...
     * @param mLayoutResId: 布局
     */
    public View setDefaultMoreFooter(Context context, int mLayoutResId) {
        View inflate = LayoutInflater.from(context).inflate(mLayoutResId, null);
     ...
     * @param mLayoutResId: 资源布局文件id
     */
    public View setDefaultEmptyView(Context context, Integer mLayoutResId) {
        View inflate = LayoutInflater.from(context).inflate(mLayoutResId, null);
    ...
    public void setEmptyView(View emptyView) {
        ...
        mIsUseEmpty = true;
        if (emptyInsert && getEmptyViewCount() == 1) {
            int position = 0;
            if (mHeadAndEmptyEnable && getHeaderLayoutCount() != 0) {
                position++;
            }
    ...
    /**
     * @param layoutResId ID for an XML layout_game_detail_outs_more_image_progress_bar resource to load
...
//            subItemCount += list.size();
     ...
     * Expand an expandable item
     *
     * @param position position of the item, which includes the header layout_game_detail_outs_more_image_progress_bar count.
     * @param animate  expand items with animation
     * @return the number of items that have been added.
     */
     ...
     *
     * @param position the position of the item, which includes the header layout_game_detail_outs_more_image_progress_bar count.

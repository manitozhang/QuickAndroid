...
import kotlinx.android.synthetic.main.activity_refresh_load_list.*
...
class RefreshLoadListActivity : BaseActivity() {
    ...
    private fun initRefreshRecycler() {
        ...
        rv_refresh!!.layoutManager = layoutManager
        refreshLoadListAdapter = RefreshLoadListAdapter(emptyList())
        rv_refresh!!.adapter = refreshLoadListAdapter
    }
    ...
    override fun initListener() {
        super.initListener()
        refresh_layout!!.setOnRefreshListener { refreshLayout: RefreshLayout ->
            //刷新,页数是1
            page = 1
            getList(page)
            refresh_layout.finishRefresh()
        }
        refresh_layout!!.setOnLoadMoreListener { refreshLayout: RefreshLayout ->
            //加载,页数往上+1
            page++
            getList(page)
